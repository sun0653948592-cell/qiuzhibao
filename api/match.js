...
export default async function handler(request, response) {
  ...
  try {
    ...
    const noPrediction = !ownPrediction?.percent;
    const homeProb = noPrediction ? 33 : pct(ownPrediction.percent.home);
    const drawProb = noPrediction ? 34 : pct(ownPrediction.percent.draw);
    const awayProb = noPrediction ? 33 : pct(ownPrediction.percent.away);
    const advice = noPrediction ? '该场比赛暂未获得足够的积分榜与主客场数据，网站会在数据更新后自动校准。' : ownPrediction.advice;
